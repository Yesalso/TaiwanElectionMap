# -*- coding: utf-8 -*-
"""
把 2016 年第九屆全國不分區及僑居國外國民立法委員選舉原始投開票所資料
（立委-A05-6-(縣市).xls）整理成仿 2008 得票率 Excel 的格式。

每個縣市產出一個 Excel 檔 (2016{縣市}_得票率.xlsx)，內含三個工作表：
  各里彙總     村里得票率（同村里多投開票所以得票數加總後再算得票率）
  各鄉鎮(市、區)彙總  鄉鎮市區層級得票數與得票率
  縣市彙總     縣市層級得票數與得票率

原始檔案格式（A05-6，與 2012 相同）：
    第0列 標題（含縣市名）
    第1列 欄位名（鄉(鎮、市、區)別 | 村里別 | 投票所別 | 各政黨得票情形 ...）
    第2列 政黨名（由第3欄起，共 N 個政黨；2016 年共 18 個政黨）
    第5列 總計（其後各列為村里投票所資料；縣市/鄉鎮小計列無村里名）
    政黨得票數後依序為：有效票數 | 無效票數 | 投票數 | ... ，欄位位置隨政黨數浮動
"""
import glob
import os
import re
...
def main():
    ...
    for path in files:
        ...
        out_path = os.path.join(OUT_DIR, f"2016{county}_得票率.xlsx")
