# -*- coding: utf-8 -*-
"""
1996 年第九屆總統副總統選舉 —— 鄉鎮市區得票率地圖

以全台「唯一色」底圖（Colorful.png）直接重著色，無需 SHP：
    - Colorful.png 中每個鄉鎮市區填以單一辨識色
    - Name_Color_Correspondence.xlsx：顏色(HEX) ↔ 鄉鎮市區 對照表
    - 得票資料：Get_data/1996總統副總統選舉_縣市鄉鎮.xlsx（鄉鎮市區級別）
      欄位：縣市、鄉鎮市區（當時名稱）＋ 四位候選人得票數／得票率

  1996 年選舉為全國性選舉，涵蓋當時之省轄縣市、臺北市、高雄市、金門縣、連江縣：
    - 縣市名為當時名稱（臺北縣／桃園縣／臺中縣／臺南縣／高雄縣…），對照表以
      現行縣市呈現，由 OLD_COUNTY_MAP 對應
    - 臺中縣＋臺中市、臺南縣＋臺南市、高雄縣＋高雄市 分別對應至現行
      台中市、台南市、高雄市
    - 臺南市 1996 年分為「中區」「西區」，對照表已併為「中西區」→ 以兩區得票率平均
    - 高雄縣三民鄉（現那瑪夏區）以來源縣市區分，與高雄市三民區不衝突

  四位候選人：李登輝／連戰（中國國民黨）、彭明敏／謝長廷（民主進步黨）、
  林洋港／郝柏村（新黨）、陳履安／王清峰（無黨籍）。

繪圖邏輯（圖例／文字／版面）沿用 1994TaiwanProvince.py / Converge_to_map.py。
...
VOTE_XLSX = os.path.join(
    os.path.dirname(BASE_DIR), "Get_data", "1996總統副總統選舉_縣市鄉鎮.xlsx"
)
OUT_PNG = os.path.join(MAPS_DIR, os.path.basename(BASE_DIR),
                       "1996年總統副總統選舉_得票率地圖.png")
...
TAG = ("1996 第九屆總統副總統選舉（中國國民黨：李登輝/連戰 / 民主進步黨：彭明敏/謝長廷 "
       "/ 新黨：林洋港/郝柏村 / 無黨籍：陳履安/王清峰）")
...
TOWN_MERGE = {   # 1996 臺南市 中區＋西區 → 對照表中西區
    ("臺南市", "中"): "中西",
    ("臺南市", "西"): "中西",
}
SPECIAL_TOWN = {   # 1996 高雄縣三民鄉 → 現那瑪夏區（與高雄市三民區區別）
    ("高雄縣", "三民"): "那瑪夏",
}
...
def parse_1996_votes():
    """1996 xlsx（鄉鎮市區級別）：縣市、鄉鎮市區 分列，四位候選人得票率並排。
...
def main():
    ...
    data, cand_names, unmatched = parse_1996_votes()
    print(f"  1996 選舉資料      : {len(data)} 個鄉鎮市區（候選人 {cand_names}）")
