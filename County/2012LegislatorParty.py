# -*- coding: utf-8 -*-
"""
2012 年第八屆全國不分區及僑居國外國民立法委員選舉 —— 鄉鎮市區得票率地圖

資料來源：2012Legislator-at-Large/data/2012{縣市}_得票率.xlsx
    （各鄉鎮(市、區)彙總 工作表：選舉區別 | 鄉(鎮、市、區)別 | <政黨>得票率 ...）
    政黨票為全國性選舉，2012 年全台各鄉鎮市區領先政黨僅中國國民黨、民主進步黨兩種。

以全台「唯一色」底圖（Colorful.png）直接重著色，無需 SHP：
    - Colorful.png 中每個鄉鎮市區填以單一辨識色
    - Name_Color_Correspondence.xlsx：顏色(HEX) ↔ 鄉鎮市區 對照表（現行行政區）

2012 年僅桃園縣尚未升格（其餘縣市制已於 2010 年調整完成），
    由 OLD_COUNTY_MAP 將 桃園縣 對應至現行 桃園市。

繪圖邏輯（圖例／文字／版面）沿用 2008LegislatorParty.py / 2001LegislatorParty.py。

執行：
    py 2012LegislatorParty.py
"""
import glob
import os
import re
...
DATA_DIR = os.path.join(os.path.dirname(BASE_DIR), "2012Legislator-at-Large", "data")
OUT_PNG = os.path.join(MAPS_DIR, os.path.basename(BASE_DIR),
                       "2012年立法委員選舉_得票率地圖.png")
...
TITLE_LINES = [
    "中華民國一百零一年一月十四日",
    "第八屆全國不分區及僑居國外國民立法委員選舉",
    "在全國各鄉鎮市區得票領先之政黨得票比例圖",
]
TAG = ("2012 第八屆全國不分區及僑居國外國民立法委員選舉"
       "（中國國民黨 / 民主進步黨）")

# ===================== 縣市名映射（2012 舊縣市 → 對照表現行縣市） =====================
OLD_COUNTY_MAP = {
    "臺北市": "臺北市", "新北市": "新北市", "桃園縣": "桃園市",
...
def parse_2012_votes():
    """2012 各縣市 xlsx（各鄉鎮彙總）：選舉區別、鄉鎮市區分列，各政黨得票率並排。
    回傳 (data, cand_names, unmatched)
      data: {(target_county, town_core): {party: rate}}
      unmatched: 縣市映射失敗而跳過的記錄
    """
    files = sorted(glob.glob(os.path.join(DATA_DIR, "2012*_得票率.xlsx")))
    if not files:
        raise FileNotFoundError(f"找不到 2012 得票率 Excel：{DATA_DIR}")
...
def main():
    ...
    data, cand_names, unmatched = parse_2012_votes()
    print(f"  2012 選舉資料      : {len(data)} 個鄉鎮市區（政黨 {len(cand_names)} 個）")
    if unmatched:
        print("  ※ 縣市映射失敗而跳過：")
        for k in unmatched:
            print(f"      {k}")
