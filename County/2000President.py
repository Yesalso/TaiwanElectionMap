# -*- coding: utf-8 -*-
"""
2000 年第十屆總統副總統選舉 —— 鄉鎮市區得票率地圖

以全台「唯一色」底圖（Colorful.png）直接重著色，無需 SHP：
    - Colorful.png 中每個鄉鎮市區填以單一辨識色
    - Name_Color_Correspondence.xlsx：顏色(HEX) ↔ 鄉鎮市區 對照表
    - 得票資料：Get_data/2000總統副總統選舉_縣市鄉鎮.xlsx（鄉鎮市區級別）
      欄位：縣市、鄉鎮市區（當時名稱）＋ 五位候選人得票數／得票率

  2000 年選舉為全國性選舉，涵蓋當時之省轄縣市、臺北市、高雄市、金門縣、連江縣：
    - 縣市名為當時名稱（臺北縣／桃園縣／臺中縣／臺南縣／高雄縣…），對照表以
      現行縣市呈現，由 OLD_COUNTY_MAP 對應
    - 臺中縣＋臺中市、臺南縣＋臺南市、高雄縣＋高雄市 分別對應至現行
      台中市、台南市、高雄市
    - 臺南市 2000 年分為「中區」「西區」，對照表已併為「中西區」→ 以兩區得票率平均
    - 高雄縣三民鄉（現那瑪夏區）以來源縣市區分，與高雄市三民區不衝突

  五位候選人：宋楚瑜／張昭雄（無黨籍）、連戰／蕭萬長（中國國民黨）、
  李敖／馮滬祥（新黨）、許信良／朱惠良（無黨籍）、陳水扁／呂秀蓮（民主進步黨）。

繪圖邏輯（圖例／文字／版面）沿用 1996President.py / 1994TaiwanProvince.py。
...
VOTE_XLSX = os.path.join(
    os.path.dirname(BASE_DIR), "Get_data", "2000總統副總統選舉_縣市鄉鎮.xlsx"
)
OUT_PNG = os.path.join(MAPS_DIR, os.path.basename(BASE_DIR),
                       "2000年總統副總統選舉_得票率地圖.png")
...
TITLE_LINES = [
    "中華民國八十九年三月十八日",
    "第十屆中華民國總統副總統選舉",
    "在全國各鄉鎮市區得票領先之候選人得票比例圖",
]
TAG = ("2000 第十屆總統副總統選舉（中國國民黨：連戰/蕭萬長 / 民主進步黨：陳水扁/呂秀蓮 "
       "/ 新黨：李敖/馮滬祥 / 無黨籍：宋楚瑜/張昭雄、許信良/朱惠良）")

# ===================== 縣市名映射（1996 當時縣市 → 對照表現行縣市） =====================
OLD_COUNTY_MAP = {
...
TOWN_MERGE = {   # 2000 臺南市 中區＋西區 → 對照表中西區
    ("臺南市", "中"): "中西",
    ("臺南市", "西"): "中西",
}
SPECIAL_TOWN = {   # 2000 高雄縣三民鄉 → 現那瑪夏區（與高雄市三民區區別）
    ("高雄縣", "三民"): "那瑪夏",
}
...
def parse_2000_votes():
    """2000 xlsx（鄉鎮市區級別）：縣市、鄉鎮市區 分列，五位候選人得票率並排。
...
def main():
    ...
    data, cand_names, unmatched = parse_2000_votes()
    print(f"  2000 選舉資料      : {len(data)} 個鄉鎮市區（候選人 {cand_names}）")
    ...
    # 固定畫布寬度（與 1996 總統地圖一致），標題過寬時向左平移而非撐寬畫布
    FIXED_W = 5071
    RIGHT_PAD = 110
    title_x = legend_x + (group_w - title_img.width) / 2
    title_x = min(title_x, FIXED_W - RIGHT_PAD - title_img.width)
