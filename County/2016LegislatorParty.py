# -*- coding: utf-8 -*-
"""
2016 年第九屆全國不分區及僑居國外國民立法委員選舉 —— 鄉鎮市區得票率地圖

資料來源：2016Legislator-at-Large/data/2016{縣市}_得票率.xlsx
    （各鄉鎮(市、區)彙總 工作表：選舉區別 | 鄉(鎮、市、區)別 | <政黨>得票率 ...）
    政黨票為全國性選舉，2016 年全台各鄉鎮市區領先政黨為
    中國國民黨、民主進步黨，另有 1 個鄉鎮（澎湖縣七美鄉）由
    無黨團結聯盟領先（粉紅色系，自 40% 起）。

以全台「唯一色」底圖（Colorful.png）直接重著色，無需 SHP：
    - Colorful.png 中每個鄉鎮市區填以單一辨識色
    - Name_Color_Correspondence.xlsx：顏色(HEX) ↔ 鄉鎮市區 對照表（現行行政區）

2016 年縣市行政區已是現行制（桃園市已於 2014 年升格），無需縣市對應調整。

繪圖邏輯（圖例／文字／版面）沿用 2012LegislatorParty.py / 2001LegislatorParty.py。

執行：
    py 2016LegislatorParty.py
"""
import glob
import os
import re
...
DATA_DIR = os.path.join(os.path.dirname(BASE_DIR), "2016Legislator-at-Large", "data")
OUT_PNG = os.path.join(MAPS_DIR, os.path.basename(BASE_DIR),
                       "2016年立法委員選舉_得票率地圖.png")
...
RATE_COLOR_STOPS = [
    ...
    [   # 第 3 组：無黨團結聯盟（粉紅色系，自 40% 起）
        (40, "#F68AB2"), (45, "#F46C9F"), (50, "#F24E8C"), (55, "#F03772"),
        (60, "#D01058"), (65, "#B30D4B"), (70, "#950B3F"), (75, "#770932"),
        (80, "#590726"),
    ],
]
...
TITLE_LINES = [
    "中華民國一百零五年一月十六日",
    "第九屆全國不分區及僑居國外國民立法委員選舉",
    "在全國各鄉鎮市區得票領先之政黨得票比例圖",
]
TAG = ("2016 第九屆全國不分區及僑居國外國民立法委員選舉"
       "（中國國民黨 / 民主進步黨 / 無黨團結聯盟）")

# ===================== 縣市名映射（2016 現行縣市 → 對照表現行縣市） =====================
OLD_COUNTY_MAP = {
...
def parse_2016_votes():
    """2016 各縣市 xlsx（各鄉鎮彙總）：選舉區別、鄉鎮市區分列，各政黨得票率並排。
    回傳 (data, cand_names, unmatched)
      data: {(target_county, town_core): {party: rate}}
      unmatched: 縣市映射失敗而跳過的記錄
    """
    files = sorted(glob.glob(os.path.join(DATA_DIR, "2016*_得票率.xlsx")))
    if not files:
        raise FileNotFoundError(f"找不到 2016 得票率 Excel：{DATA_DIR}")
...
def main():
    ...
    data, cand_names, unmatched = parse_2016_votes()
    print(f"  2016 選舉資料      : {len(data)} 個鄉鎮市區（政黨 {len(cand_names)} 個）")
    if unmatched:
        print("  ※ 縣市映射失敗而跳過：")
        for k in unmatched:
            print(f"      {k}")
